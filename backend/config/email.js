...
const sendOTPEmail = async (email, otp) => {
  const emailData = {
    sender: { name: 'StayIndia', email: process.env.BREVO_SENDER_EMAIL },
    to: [{ email: email }],
    subject: 'Password Reset OTP - StayIndia',
    htmlContent: `
      <!DOCTYPE html>
      <html>
      <head>
...
const sendBookingConfirmationEmail = async (userEmail, userName, bookingDetails) => {
  ...
  const emailData = {
    ...
    subject: '✅ Booking Confirmed - StayIndia',
    htmlContent: `
      <!DOCTYPE html>
      <html>
      <head>
...
const sendCheckInReminderEmail = async (userEmail, userName, bookingDetails) => {
  ...
  const emailData = {
    sender: { name: 'StayIndia', email: process.env.BREVO_SENDER_EMAIL },
    to: [{ email: userEmail, name: userName }],
    subject: '🔔 Check-in Reminder - Your Stay Begins Today!',
    htmlContent: `
      <!DOCTYPE html>
      <html>
      <head>
